...
public class PlottingTesting : MonoBehaviour
{
	...
	void Start()
	{
		...
		float localWidth = transform.Find("BezierFollow/Base").localScale.x;
		// -n/2...0...n/2
		for (int i = 0; i < displayWindowSize; i++)
		{
			//Instantiate point
			Transform t = (Instantiate(Resources.Load("BezierFollow"), transform) as GameObject).transform;
	...
	public void updatePlot(List<SpectralFluxInfo2> pointInfo, int curIndex = -1)
	{
		...
		for (int i = windowStart; i < windowEnd; i++)
		{
			...
			Transform threshPoint = plotPoints[plotIndex].Find("p0");
			//Transform peakPoint = plotPoints[plotIndex].Find ("PeakPoint");


			if (pointInfo[i].isPeak)
			{
				//setPointHeight(peakPoint, pointInfo[i].threshold + 0.05f);
